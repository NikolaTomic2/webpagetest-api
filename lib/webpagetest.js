...
var reSpace = /\s/,
  reConnectivity =
    /^(?:Cable|DSL|3GSlow|3G|3GFast|4GSlow|4G|4GFast|LTE|Edge|2G|Dial|FIOS|Native|custom)$/,
      ...
      if (statusCode !== 200) {
        ...
        res.on("end", function onEnd() {
          ...
          for (i = 0, len = data.length, pos = 0; i < len; i += 1) {
            console.log("loop buffer --> ", i, buffer.length);
            data[i].copy(buffer, pos);
            pos += data[i].length;
          }

          console.log("compressed buffer --> ", buffer.length);
          ...
          if (encoding === "gzip" || encoding === "deflate") {
            ...
            zlib.unzip(buffer, function unzip(err, buffer) {
              if (err) {
                ...
                console.log("decompressed buffer --> ", buffer.length);
